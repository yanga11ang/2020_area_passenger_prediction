...
class Trainer(object):
	...
	def train_one_step(self, data):
		print("in Trainer train_one_step\n")
		self.optimizer.zero_grad()
		# 负采样model
		loss = self.model({
			'batch_x': self.to_var(data['batch_x'], self.use_gpu),
			'batch_y': self.to_var(data['batch_y'], self.use_gpu),
			'batch_id': self.to_var(data['batch_id'], self.use_gpu),
		}) # size is 1
		loss.backward()
		self.optimizer.step()
		return loss.item()
	...
	def run(self):
		...
		for epoch in training_range: # 迭代 train_times 次 epoch
			...
			for data in self.data_loader: # 迭代 nbatches 次 batch
				# data 是一个字典，
				# 			"batch_x": self.batch_x,  size [batches_size * x_dim] np
				# 			"batch_y": self.batch_y,
				# 			"batch_id": self.batch_id, 是那个区域
				# 			"hour": hour 时间, 是几点钟
